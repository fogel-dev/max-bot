...
function basicAuth(req, res, next) {
    ...
    if (username !== process.env.WEBHOOK_BASIC_USER ||
...
app.post('/events', basicAuth, async (req, res) => {
    ...
    try {
        await bot.api.sendMessageToUser(Number(user.user_id), data.message);
    }
    catch (e) {
        // @ts-ignore
        if (e.status === 403) {
...
bot.command('esia', async (ctx) => {
    return ctx.reply('Чтобы авторизоваться в ЕСИА, нажмите на кнопку ниже:', {
        attachments: [
            Keyboard.inlineKeyboard([
                [Keyboard.button.link('Авторизоваться в ЕСИА', 'https://esia-portal1.test.gosuslugi.ru/aas/oauth2/ac?client_id=FD5470')],
